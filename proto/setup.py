...
"""TODO: kenluo - DO NOT SUBMIT without one-line documentation for setup.

TODO: kenluo - DO NOT SUBMIT without a detailed description of setup.
"""
...
class ProtoBuild(Command):
  ...
  def _compile_proto_in_place(self, includes):
    if not self.build_lib:
      print("Skipping Unified Schema compilation since build_lib is not set")
      return
  ...
  def run(self):
    ...
    with TemporaryDirectory() as t:
      ...
      if self._pull_deps(temp_root):
        return
      inlcudes = [self._root] + [temp_root / path for path in self._deps.keys()]
      if self._compile_proto_in_place(inlcudes):
        return


class CustomBuild(build):
  sub_commands = [("compile_unified_schema_proto", None)] + build.sub_commands
