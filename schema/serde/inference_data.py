...
class InferenceDataSerde(
  ...
  def serialize(self, obj: az.InferenceData) -> meridian_pb.InferenceData:
    ...
    if hasattr(obj, _PRIOR_FIELD):
      ...
      prior_bytes = prior_dataset_copy.to_netcdf(format=_NETCDF_FORMAT)  # pytype: disable=attribute-error
    else:
      prior_bytes = None
    ...
    if hasattr(obj, _POSTERIOR_FIELD):
      ...
      posterior_bytes = posterior_dataset_copy.to_netcdf(format=_NETCDF_FORMAT)  # pytype: disable=attribute-error
    else:
    ...
    for group in obj.groups():
      ...
      aux[group] = aux_dataset_copy.to_netcdf(format=_NETCDF_FORMAT)
