...
def _mock_infdata_with_datasets(
  ...
  if include_prior:
    ...
    prior.to_netcdf.return_value = b"test-prior-bytes"
  ...
  if include_posterior:
    ...
    posterior.to_netcdf.return_value = b"test-posterior-bytes"
    ...
    sample_stats.to_netcdf.return_value = b"test-stats-bytes"
    ...
    trace.to_netcdf.return_value = b"test-trace-bytes"
...
class InferenceDataTest(parameterized.TestCase):
  ...
  def test_serialize(
    ...
    compare.assertProto2Equal(self, infdata_proto, expected_infdata_proto)
