...
"""Defines a processor for budget optimization."""
...
from proto.mmm.v1.common import target_metric_pb2 as target_pb
from proto.mmm.v1.marketing.analysis import kpi_outcome_pb2 as kpi_outcome_pb
from proto.mmm.v1.marketing.analysis import marketing_analysis_pb2 as analysis_pb
from proto.mmm.v1.marketing.analysis import media_analysis_pb2 as media_analysis_pb
...
class BudgetOptimizationProcessor(
  ...
  def execute(
    ...
    for spec in specs:
      ...
      constraints_kwargs = self._build_constraints_kwargs(spec)
      ...
      if spec.new_data is not None and spec.new_data.time is not None:
        time_coords = tc.TimeCoordinates.from_dates(
            [s.decode() for s in spec.new_data.time.numpy()]
        )
      else:
...
def _to_kpi_outcome(channel_data: xr.Dataset) -> kpi_outcome_pb.KpiOutcome:
  ...
  return kpi_outcome_pb.KpiOutcome(
      kpi_type=(
      ...
      contribution=kpi_outcome_pb.Contribution(
          value=_to_estimate(
              channel_data.incremental_outcome, confidence_level
          ),
      ),
      effectiveness=kpi_outcome_pb.Effectiveness(
