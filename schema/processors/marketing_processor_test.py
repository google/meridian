...
_ALL_PAID_CHANNELS = ["channel_1", "channel_2"]
_ALL_NON_PAID_CHANNELS = ["organic_channel_1", "non_media_channel_1"]
_ALL_CHANNELS = _ALL_PAID_CHANNELS + _ALL_NON_PAID_CHANNELS

_ALL_CHANNEL_IDX = 0
_BASELINE_CHANNEL_IDX = 1
...
_NUMBER_OF_PAID_CHANNELS = (
    len(_ALL_PAID_CHANNELS) + 1
)  # +1 for the pseudo "all channels"
_NUMBER_OF_ALL_CHANNELS = (
    len(_ALL_CHANNELS) + 1
)  # +1 for the pseudo "all channels"
...
_SPEND_DATA = test_utils.SAMPLE_SPEND[0:_NUMBER_OF_PAID_CHANNELS]
_PCT_OF_SPEND_DATA = test_utils.SAMPLE_PCT_OF_SPEND[0:_NUMBER_OF_PAID_CHANNELS]
_EFFECTIVENESS_DATA = test_utils.SAMPLE_EFFECTIVENESS[
    0:_NUMBER_OF_PAID_CHANNELS
]
...
_ROI_DATA = test_utils.SAMPLE_ROI[0:_NUMBER_OF_PAID_CHANNELS]
_MROI_DATA = test_utils.SAMPLE_MROI[0:_NUMBER_OF_PAID_CHANNELS]
_CPIK_DATA = test_utils.SAMPLE_CPIK[0:_NUMBER_OF_PAID_CHANNELS]
...
_MEDIA_SUMMARY_SPEC_WITH_NON_PAID_CHANNELS = marketing_processor.MarketingAnalysisSpec(
    media_summary_spec=marketing_processor.MediaSummarySpec(
...
def _create_response_curve_data() -> xr.Dataset:
  ...
  xr_coords = {
      constants.CHANNEL: ([constants.CHANNEL], _ALL_PAID_CHANNELS),
      constants.METRIC: (
          [constants.METRIC],
...
def _create_media_summary_metrics_aggregated_data(
  ...
  xr_coords_with_ci_and_distribution = {
      constants.CHANNEL: (
          [constants.CHANNEL],
          _ALL_PAID_CHANNELS + [constants.ALL_CHANNELS],
      ),
      constants.METRIC: (
          [constants.METRIC],
          [constants.MEAN, constants.MEDIAN, constants.CI_LO, constants.CI_HI],
...
def _create_media_summary_metrics_data_with_time_breakdown() -> xr.Dataset:
  ...
  xr_data = {
      constants.SPEND: (
          xr_dims,
          np.full([_NUMBER_OF_TIMES, _NUMBER_OF_PAID_CHANNELS], 10),
      ),
      constants.PCT_OF_SPEND: (
          xr_dims,
          np.full([_NUMBER_OF_TIMES, _NUMBER_OF_PAID_CHANNELS], 20),
      ),
      constants.INCREMENTAL_OUTCOME: (
          xr_dims_with_ci_and_distribution,
          np.full([_NUMBER_OF_TIMES, _NUMBER_OF_PAID_CHANNELS, 4, 2], 60),
      ),
      constants.PCT_OF_CONTRIBUTION: (
          xr_dims_with_ci_and_distribution,
          np.full([_NUMBER_OF_TIMES, _NUMBER_OF_PAID_CHANNELS, 4, 2], 70),
      ),
  }
...
def _create_expected_media_analysis(
  ...
  channel_name = (
      constants.ALL_CHANNELS if index == 2 else _ALL_PAID_CHANNELS[index]
  )
...
class MarketingProcessorTest(parameterized.TestCase):
  ...
  def setUp(self):
    ...
    self.mock_meridian_model.all_channel_names = _ALL_PAID_CHANNELS
    self.mock_meridian_model.input_data.media_channel = _ALL_PAID_CHANNELS
  ...
  def test_execute_incremental_outcome(self):
    n_chains = 2
    n_draws = 3
    n_channels = _NUMBER_OF_PAID_CHANNELS
    ...
    self.mock_meridian_model.input_data.time = all_times_dataarray
    # As we added non-media breakdowns, keep this mock for coverage.
    self.mock_meridian_model.input_data.get_all_paid_channels.return_value = (
        _ALL_PAID_CHANNELS
    )
    self.mock_meridian_model.input_data.get_all_channels.return_value = (
        _ALL_CHANNELS
    )
    marketing_analysis_list = marketing_processor.MarketingProcessor(
        trained_model=self.mock_trained_model,
    ...
    for channel_name in _ALL_CHANNELS:
      if channel_name in _ALL_PAID_CHANNELS:
        expected_marketing_analysis.media_analyses.append(
            _create_media_analysis_inc_outcome(
                channel_name=channel_name, fill_value=1.0
            )
        )
    expected_marketing_analysis.media_analyses.append(
        _create_media_analysis_inc_outcome(
  ...
  def test_execute_incremental_outcome_non_revenue_metrics_no_revenue_per_kpi(
    ...
    for channel_name in _ALL_PAID_CHANNELS:
      expected_marketing_analysis.media_analyses.append(
          _create_media_analysis_inc_outcome(
              channel_name=channel_name,
              fill_value=1.0,
  ...
  def test_execute_incremental_outcome_with_new_data_not_aggregated(self):
    ...
    for channel_name in _ALL_PAID_CHANNELS:
      expected_media_analyses.append(
          _create_media_analysis_inc_outcome(
              channel_name=channel_name, fill_value=1.0
          )
      )
  ...
  def test_execute_incremental_outcome_with_new_data_with_selected_times(self):
    ...
    self.mock_analyzer.compute_incremental_outcome_aggregate.return_value = (
        tf.convert_to_tensor(np.ones((2, 3, _NUMBER_OF_PAID_CHANNELS)))
    )
    # As we added non-media breakdowns, keep this mock for coverage.
    self.mock_meridian_model.input_data.get_all_paid_channels.return_value = (
        _ALL_PAID_CHANNELS
