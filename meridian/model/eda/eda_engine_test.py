...
class EDAEngineTest(
  ...
  def test_check_treatment_control_variability_raises_for_national_model(self):
    meridian = model.Meridian(self.national_input_data_media_and_rf)
    engine = eda_engine.EDAEngine(meridian)
    with self.assertRaisesRegex(
        ValueError,
        "check_treatment_control_variability is not supported for national"
        " models.",
    ):
      engine.check_treatment_control_variability()

  def test_check_treatment_control_variability_geo_model_output_correct(self):
    meridian = model.Meridian(self.input_data_with_media_and_rf)
    engine = eda_engine.EDAEngine(meridian)
    outcome = engine.check_treatment_control_variability()

    with self.subTest("check_type"):
      self.assertEqual(
          outcome.check_type,
          eda_outcome.EDACheckType.TREATMENT_CONTROL_VARIABILITY,
      )
    with self.subTest("findings"):
    ...
    with self.subTest("analysis_artifacts"):
      ...
      artifact = outcome.analysis_artifacts[0]
      self.assertIsInstance(
          artifact, eda_outcome.TreatmentControlVariabilityArtifact
      )
      self.assertEqual(artifact.level, eda_outcome.AnalysisLevel.OVERALL)
  ...
  def test_check_treatment_control_variability_geo_model_r2_values_correct(
      self,
  ):
    meridian = model.Meridian(self.input_data_with_media_only)
    engine = eda_engine.EDAEngine(meridian)
