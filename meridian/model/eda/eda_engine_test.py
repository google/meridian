...
class EDAEngineTest(
  ...
  def test_check_std_geo_finds_zero_std_kpi(self):
    meridian = mock.Mock(spec=model.Meridian)
    meridian.is_national = False
    engine = eda_engine.EDAEngine(meridian)
  ...
  def test_check_std_geo_finds_zero_std_treatment_control(self):
    meridian = mock.Mock(spec=model.Meridian)
    meridian.is_national = False
    engine = eda_engine.EDAEngine(meridian)
  ...
  def test_check_std_geo_finds_zero_std_reach(self):
    meridian = mock.Mock(spec=model.Meridian)
    meridian.is_national = False
    engine = eda_engine.EDAEngine(meridian)
  ...
  def test_check_std_geo_finds_zero_std_freq(self):
    meridian = mock.Mock(spec=model.Meridian)
    meridian.is_national = False
    engine = eda_engine.EDAEngine(meridian)
  ...
  def test_check_std_national_finds_zero_std_treatment_control(self):
    meridian = mock.Mock(spec=model.Meridian)
    meridian.is_national = True
    engine = eda_engine.EDAEngine(meridian)
    ...
    self.assertIn(
        "The standard deviation of these scaled treatment or control variables"
        " drops from positive to zero",
        outcome.findings[0].explanation,
    )

  def test_check_std_national_finds_zero_std_reach(self):
    meridian = mock.Mock(spec=model.Meridian)
    meridian.is_national = True
    engine = eda_engine.EDAEngine(meridian)
    ...
    self._mock_eda_engine_property(
        "national_all_reach_scaled_da", mock_reach_da
    )
    self._mock_eda_engine_property("national_all_freq_da", None)
    outcome = engine.check_std_national()
    self.assertLen(outcome.findings, 1)
  ...
  def test_check_std_national_finds_zero_std_freq(self):
    meridian = mock.Mock(spec=model.Meridian)
    meridian.is_national = True
    engine = eda_engine.EDAEngine(meridian)
    ...
    self._mock_eda_engine_property("national_all_reach_scaled_da", None)
    self._mock_eda_engine_property("national_all_freq_da", mock_freq_da)
    outcome = engine.check_std_national()
    self.assertLen(outcome.findings, 1)
    self.assertEqual(
        outcome.findings[0].severity, eda_outcome.EDASeverity.ATTENTION
