...
class EDAEngineTest(
  ...
  def test_check_stdev_geo_raises_error_for_national_model(self):
    meridian = mock.Mock(spec=model.Meridian)
    meridian.is_national = True
    engine = eda_engine.EDAEngine(meridian)

    with self.assertRaisesRegex(
        ValueError, "check_stdev_geo is not applicable for national models."
    ):
      engine.check_stdev_geo()

  def test_check_stdev_geo_correctly_identifies_outliers(self):
    meridian = model.Meridian(self.input_data_with_media_only)
    engine = eda_engine.EDAEngine(meridian)

    kpi_data = np.array([[10, 11, 12, 11, 10, 100, 11]], dtype=float)
    ...
    outcome = engine.check_stdev_geo()
    ...
    self.assertGreater(
        kpi_result.std_ds[eda_engine._STDEV_WITH_OUTLIERS_VAR_NAME].values[0],
        kpi_result.std_ds[eda_engine._STDEV_WITHOUT_OUTLIERS_VAR_NAME].values[
            0
        ],
    )
    self.assertFalse(kpi_result.outlier_df.empty)
    self.assertEqual(
        kpi_result.outlier_df[eda_engine._OUTLIERS_COL_NAME].iloc[0], 100
    )

  def test_check_stdev_geo_returns_info_finding_when_no_issues(self):
    meridian = mock.Mock(spec=model.Meridian)
  ...
  def test_check_stdev_geo_finds_zero_std_kpi(self):
    meridian = mock.Mock(spec=model.Meridian)
    meridian.is_national = False
    engine = eda_engine.EDAEngine(meridian)

    mock_kpi_da = _create_single_var_data_array(
        np.ones((1, 7), dtype=float),
        name=constants.KPI_SCALED,
    )
    ...
    self.assertIn(
        "KPI has zero standard deviation", outcome.findings[0].explanation
    )

  def test_check_stdev_geo_finds_zero_std_treatment_control(self):
    meridian = mock.Mock(spec=model.Meridian)
    meridian.is_national = False
    engine = eda_engine.EDAEngine(meridian)
  ...
  def test_check_stdev_geo_finds_zero_std_reach(self):
    meridian = mock.Mock(spec=model.Meridian)
    meridian.is_national = False
    engine = eda_engine.EDAEngine(meridian)
    ...
    mock_reach_da = _create_single_var_data_array(
        np.ones((1, 7, 1), dtype=float),
    ...
    self._mock_eda_engine_property("kpi_scaled_da", mock_kpi_da)
    self._mock_eda_engine_property("treatment_control_scaled_ds", mock_tc_ds)
    self._mock_eda_engine_property("all_reach_scaled_da", mock_reach_da)
    self._mock_eda_engine_property("all_freq_da", None)
    outcome = engine.check_stdev_geo()
    self.assertLen(outcome.findings, 1)
    self.assertEqual(
    ...
    self.assertIn(
        "zero variation of reach across time",
        outcome.findings[0].explanation,
    )

  def test_check_stdev_geo_finds_zero_std_freq(self):
    meridian = mock.Mock(spec=model.Meridian)
    meridian.is_national = False
    engine = eda_engine.EDAEngine(meridian)
    ...
    mock_freq_da = _create_single_var_data_array(
        np.ones((1, 7, 1), dtype=float),
    ...
    self._mock_eda_engine_property("treatment_control_scaled_ds", mock_tc_ds)
    self._mock_eda_engine_property("all_reach_scaled_da", None)
    self._mock_eda_engine_property("all_freq_da", mock_freq_da)
    outcome = engine.check_stdev_geo()
    self.assertLen(outcome.findings, 1)
    self.assertEqual(
        outcome.findings[0].severity, eda_outcome.EDASeverity.ATTENTION
    ...
    self.assertIn(
        "zero variation of frequency across time",
        outcome.findings[0].explanation,
    )

  def test_check_stdev_geo_handles_missing_rf_data(self):
    meridian = mock.Mock(spec=model.Meridian)
    meridian.is_national = False
    engine = eda_engine.EDAEngine(meridian)
