...
class EDAEngineTest(
  ...
  def test_check_cost_per_media_unit_artifact_values(self, is_national):
    meridian = mock.Mock(spec=model.Meridian)
    meridian.is_national = is_national
    engine = eda_engine.EDAEngine(meridian)
    ...
    if is_national:
      ...
      check_method = engine.check_national_cost_per_media_unit
    else:
      shape = (1, 10, 1)
      level = eda_outcome.AnalysisLevel.GEO
      spend_ds_prop = "all_spend_ds"
      media_unit_ds_prop = "paid_raw_media_units_ds"
      check_method = engine.check_geo_cost_per_media_unit
