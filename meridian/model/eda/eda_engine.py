...
_Q3_THRESHOLD = 0.75
_IQR_MULTIPLIER = 1.5
_STDEV_WITH_OUTLIERS_VAR_NAME = 'stdev_with_outliers'
_STDEV_WITHOUT_OUTLIERS_VAR_NAME = 'stdev_without_outliers'
_OUTLIERS_COL_NAME = 'outliers'
_ABS_OUTLIERS_COL_NAME = 'abs_outliers'


# A custom error class for the case where a geo level check is called but the
# model is national.
...
def _calculate_stdev(
    input_da: xr.DataArray,
) -> tuple[xr.Dataset, pd.DataFrame]:
  """Helper function to compute stdev with and without outliers.

  Args:
    input_da: A DataArray for which to calculate the stdev.

  Returns:
    A tuple where the first element is a Dataset with two data variables:
    'stdev_incl_outliers' and 'stdev_excl_outliers'. The second element is a
    DataFrame with columns for variables, geo (if applicable), time, and
    outlier values.
  """
  stdev_with_outliers = input_da.std(dim=constants.TIME, ddof=1)
  ...
  stdev_without_outliers = da_no_outlier.std(dim=constants.TIME, ddof=1)
...
class EDAEngine:
  ...
  def _check_stdev(
    ...
    std_ds, outlier_df = _calculate_stdev(data)

    finding = None
    if (std_ds[_STDEV_WITH_OUTLIERS_VAR_NAME] == 0).any():
  ...
  def check_stdev_geo(
    ...
    """Checks stdev for geo-level KPI, treatments, R&F, and controls."""
    if self._meridian.is_national:
      raise ValueError('check_stdev_geo is not applicable for national models.')
    ...
    data_arg_name = 'data'
    message_arg_name = 'zero_std_message'

    da_kwargs_list = [
        {
            data_arg_name: self.kpi_scaled_da,
            message_arg_name: (
        ...
        {
            data_arg_name: treatment_control_scaled_da,
            message_arg_name: (
        ...
        {
            data_arg_name: self.all_reach_scaled_da,
            message_arg_name: (
        ...
        {
            data_arg_name: self.all_freq_da,
            message_arg_name: (
    ...
    for kwargs in da_kwargs_list:
      if kwargs[data_arg_name] is None:
        continue
      finding, result = self._check_stdev(
          level=eda_outcome.AnalysisLevel.GEO,
          **kwargs,
      )
