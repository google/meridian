...
class EDAEngine:
  ...
  def check_treatment_control_variability(
      self,
  ) -> eda_outcome.EDAOutcome[eda_outcome.TreatmentControlVariabilityArtifact]:
    """Compute adjusted R-squared for treatments and controls vs geo and time.

    These checks are applied to geo-level dataset only.

    Returns:
      An EDAOutcome object containing a TreatmentControlVariabilityArtifact.
    ...
    findings = [
        eda_outcome.EDAFinding(
            ...
            explanation=(
                ...
                ' R-squared or reducing the number of knots for mu_t.'
            ),
        ),
        eda_outcome.EDAFinding(
            severity=eda_outcome.EDASeverity.INFO,
    ...
    return eda_outcome.EDAOutcome(
        check_type=eda_outcome.EDACheckType.TREATMENT_CONTROL_VARIABILITY,
        findings=findings,
        analysis_artifacts=[artifact],
    )
