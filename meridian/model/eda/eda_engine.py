...
class EDAEngine:
  ...
  @functools.cached_property
  def media_raw_da_nat(self) -> xr.DataArray | None:
  ...
  @functools.cached_property
  def media_scaled_da_nat(self) -> xr.DataArray | None:
  ...
  def organic_media_scaled_da(self) -> xr.DataArray | None:
    if self._meridian.input_data.organic_media is None:
      return None
    organic_media_scaled_da = self._meridian.input_data.organic_media.copy()
    organic_media_scaled_da.values = (
        self._meridian.organic_media_tensors.organic_media_scaled
    )
    return self._truncate_media_time(organic_media_scaled_da)

  @functools.cached_property
  def organic_media_raw_da_nat(self) -> xr.DataArray | None:
  ...
  @functools.cached_property
  def organic_media_scaled_da_nat(self) -> xr.DataArray | None:
  ...
  def non_media_scaled_da(self) -> xr.DataArray | None:
    if self._meridian.input_data.non_media_treatments is None:
      return None
    non_media_scaled_da = self._meridian.input_data.non_media_treatments.copy()
    non_media_scaled_da.values = self._meridian.non_media_treatments_normalized
  ...
  def rf_spend_da(self) -> xr.DataArray | None:
    if self._meridian.input_data.rf_spend is None:
      return None
    rf_spend_da = self._meridian.input_data.rf_spend.copy()
    rf_spend_da.values = self._meridian.rf_tensors.rf_spend
    return rf_spend_da
  ...
  def _aggregate_and_scale_geo_da(
    ...
    """Aggregate geo-level xr.DataArray to national level with scaled values.
    ...
    Returns:
      An xr.DataArray representing the aggregated and scaled national-level
        data.
    """
    temp_geo_dim = 'national_geo'

    if da_var_agg_map is None:
      da_var_agg_map = {}

    if channel_dim is not None:
      da_nat = self._aggregate_variables(da_geo, channel_dim, da_var_agg_map)
    else:
    ...
    da_nat = da_nat.assign_coords({constants.GEO: [temp_geo_dim]})
    da_nat.values = tf.cast(da_nat.values, tf.float32)
    da_nat = self._scale_xarray(da_nat, transformer_class)

    return da_nat.sel({constants.GEO: temp_geo_dim}, drop=True)
