...
def _spearman_coeff(x, y):
  """Helper for apply_ufunc to compute spearman correlation."""
...
class EDAEngine:
  ...
  def check_population_corr(
      self,
  ) -> eda_outcome.EDAOutcome[eda_outcome.PopulationCorrelationArtifact]:
    """Checks Spearman correlation between population and media/controls.
    ...
    Raises:
      GeoLevelCheckOnNationalModelError: If the model is national.
      ValueError: If population data is missing.
    """
    if self._is_national_data:
      raise GeoLevelCheckOnNationalModelError(
          'check_population_corr is not supported for national models.'
      )

    if self.geo_population_da is None:
      raise ValueError(
