...
class MeridianEdaTest(parameterized.TestCase):
  ...
  def test_plot_cost_per_media_unit_time_series_geos(self):
    ...
    self._meridian.eda_engine.paid_raw_impressions_ds = xr.Dataset(
        {
            'media': (
                [constants.GEO, constants.TIME, constants.MEDIA_CHANNEL],
                np.array([
    ...
    mock_artifact = mock.MagicMock(spec=eda_outcome.CostPerImpressionArtifact)
    mock_artifact.cost_per_impression_da = xr.DataArray(
        np.array([
  ...
  def test_plot_cost_per_media_unit_time_series_nationalize(self):
    ...
    self._meridian.eda_engine.national_paid_raw_impressions_ds = xr.Dataset(
        {
            'media': (
                [constants.GEO, constants.TIME, constants.MEDIA_CHANNEL],
    ...
    with mock.patch.object(
        self._meridian.eda_engine,
        'check_national_cost_per_impression',
        return_value=mock_outcome,
    ):
  ...
  def test_plot_cost_per_media_unit_time_series_national_model(self):
    ...
    self._meridian.eda_engine.check_national_cost_per_impression.return_value.get_national_artifact = (
        mock_artifact
    )
