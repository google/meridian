...
class MeridianEDA:
  ...
  def plot_cost_per_media_unit_time_series(self, geos: Geos = 1) -> alt.Chart:
    ...
    if use_national_data:
      ...
      media_unit_data = (
          self._meridian.eda_engine.national_paid_raw_impressions_ds
      )
      cost_per_media_unit_artifact = (
          self._meridian.eda_engine.check_national_cost_per_impression().get_national_artifact
      )
      select_data = lambda data, geo: data
    else:
      cost_data = self._meridian.eda_engine.all_spend_ds
      media_unit_data = self._meridian.eda_engine.paid_raw_impressions_ds
      cost_per_media_unit_artifact = (
          self._meridian.eda_engine.check_geo_cost_per_impression().get_geo_artifact
      )
    ...
    for geo_to_plot in geos_to_plot:
      ...
      cost_per_media_unit_plot_data = _process_stacked_ds(
          select_data(
              cost_per_media_unit_artifact.cost_per_impression_da, geo_to_plot
          )
