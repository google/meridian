...
class Meridian:
  ...
  @functools.cached_property
  def eda_outcomes(self) -> list[eda_outcome.EDAOutcome]:
  ...
  def _run_model_training_guardail(self):
    """Raises an error if the model has critical EDA issues."""
    error_findings_by_type = collections.defaultdict(list)
    ...
    if error_findings_by_type:
      ...
      for check_type, explanations in error_findings_by_type.items():
        error_message_lines.append(f"Check type: {check_type}")
  ...
  def sample_posterior(
    ...
    """
    self._run_model_training_guardail()
