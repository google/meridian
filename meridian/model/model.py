...
class Meridian:
  ...
  def thin_posterior(
    ...
    Systematic MCMC posterior thinning selects posterior samples from each
    MCMC chain at a fixed, regular interval (such as every 10th sample) starting
    from a randomly chosen point. This is done to minimize the auto-correlation
    of the sample.
    ...
    attrs.update({
        ...
        constants.POSTERIOR_THINNING_RATE: (
            float(sampling_rate)
            if sampling_rate is not None
        ...
        constants.POSTERIOR_THINNED_DRAW_COUNT_PER_CHAIN: n_selected_draws,
    })
    if seed is not None:
  ...
  def restore_full_posterior(self) -> xr.Dataset:
    """Restores the full posterior saved by `thin_posterior()`."""
