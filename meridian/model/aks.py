...
def aspline(
  ...
  Args:
    ...
    pen: A vector of positive penalty values. The adaptive spline regression is
      performed for every value of pen
  ...
  xx_rot = np.concat(
      [
          _mat2rot(xx + (1e-20 * np.identity(ncol))),
  ...
  for _ in range(max_iter):
    par = _wridge_solver(xx_rot, xy, degree, pen[ind_pen], w, old_par=par)
...
def _mat2rot(band_mat: np.ndarray) -> np.ndarray:
...
def _band_weight(w: np.ndarray, diff: int) -> np.ndarray:
  ...
  Returns:
    ...
    superdiagonals of the weight matrix are returned, each column corresponding
    to a diagonal.
  """
  ws = len(w)

  # Compute the entries of the difference matrix
  binom = np.zeros(diff + 1, dtype=np.int32)
  for i in range(diff + 1):
    binom[i] = math.comb(diff, i) * (-1) ** i

  # Compute the limit indices
  ind_mat = np.zeros([ws + diff, 2], dtype=np.int32)
  for ind in range(ind_mat.shape[0]):
    ind_mat[ind, 0] = 0 if ind - diff < 0 else ind - diff
    ind_mat[ind, 1] = ind if ind < ws - 1 else ws - 1

  # Main loop
  result = np.zeros([ws + diff, diff + 1])
  for j in range(diff + 1):
    for i in range(ws + diff - j):
      temp = 0.0
...
def _ldl(rot_mat: np.ndarray) -> np.ndarray:
  ...
  Args:
    rot_mat: Rotated row-wised matrix of dimensions n*k, with first column
      corresponding to the diagonal, the second to the first super-diagonal and
      so on.
...
def _bandsolve(
    rot_mat: np.ndarray, rhs_mat: np.ndarray | None = None
) -> np.ndarray:
  ...
  Args:
    rot_mat: Band square matrix in the rotated form. It's the visual rotation by
      90 degrees of the matrix, where subdiagonal are discarded.
...
def _wridge_solver(
  ...
  Args:
    xx_rot: The matrix X'X where X is the design matrix. This argument is given
      in the form of a band matrix, i.e., successive columns represent
      superdiagonals.
  ...
  def _hessian_solver(par, xx_rot, xy, pen, w, diff):
    ...
    Args:
      ...
      pen: Positive penalty constant.
      w: Vector of weights.
      diff: The order of the differences of the parameter. Equals degree + 1 in
        adaptive spline regression.
    ...
    return _bandsolve(xx_rot + pen * _band_weight(w, diff), xy)[:, 0] - par
