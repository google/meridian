...
class ModelSpecTest(parameterized.TestCase):
  ...
  def test_spec_inits_valid_roi_calibration_period_works(self):
    shape = (3, 7)
    model_spec = spec.ModelSpec(
  ...
  def test_spec_inits_invalid_roi_calibration_period_fails(
  ...
  def test_spec_inits_invalid_rf_roi_calibration_period_fails(
      self, shape, error_message
  ):
    with self.assertRaisesWithLiteralMatch(ValueError, error_message):
      spec.ModelSpec(rf_roi_calibration_period=np.random.normal(size=shape))

  def test_spec_inits_disallowed_roi_calibration_period_fails(self):
    shape = (3, 7)
    with self.assertRaisesWithLiteralMatch(
        ValueError,
  ...
  def test_spec_inits_disallowed_rf_roi_calibration_period_fails(self):
    shape = (3, 7)
    with self.assertRaisesWithLiteralMatch(
        ValueError,
