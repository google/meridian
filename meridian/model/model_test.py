...
class ModelTest(
  ...
  def _meridian_with_posterior(self, posterior: xr.Dataset) -> model.Meridian:
    meridian = model.Meridian(input_data=self.input_data_with_media_only)
    meridian.inference_data.add_groups({"posterior": posterior})
    return meridian

  def test_thin_posterior_preserves_chains(self):
    ...
    thinned = meridian.thin_posterior(n_draws=4, seed=7)
  ...
  def test_thin_posterior_accepts_thinning_method_enum(self):
    meridian = self._meridian_with_posterior(_simple_posterior())

    thinned = meridian.thin_posterior(
  ...
  def test_thin_posterior_supports_non_leading_draw_dimension(self):
  ...
  def test_thin_posterior_seed_reproducible(self):
    ...
    first = first_meridian.thin_posterior(n_draws=5, seed=7)
    second = second_meridian.thin_posterior(n_draws=5, seed=7)
  ...
  def test_thin_posterior_requires_posterior(self):
    meridian = model.Meridian(input_data=self.input_data_with_media_only)

    with self.assertRaises(model.NotFittedModelError):
      meridian.thin_posterior(sampling_rate=0.1)

  @parameterized.named_parameters(
      dict(testcase_name="missing", kwargs={}),
      dict(testcase_name="both", kwargs={"sampling_rate": 0.1, "n_draws": 2}),
  )
  def test_thin_posterior_requires_exactly_one_draw_argument(
      self, kwargs
  ):
    meridian = self._meridian_with_posterior(_simple_posterior())

    with self.assertRaisesRegex(ValueError, "Exactly one"):
      meridian.thin_posterior(**kwargs)

  @parameterized.named_parameters(
      dict(testcase_name="zero", kwargs={"n_draws": 0}),
      dict(testcase_name="too_many", kwargs={"n_draws": 11}),
  )
  def test_thin_posterior_rejects_invalid_n_draws(self, kwargs):
    meridian = self._meridian_with_posterior(_simple_posterior())
  ...
  def test_thin_posterior_rejects_invalid_sampling_rate(self, kwargs):
    meridian = self._meridian_with_posterior(_simple_posterior())

    with self.assertRaisesRegex(ValueError, "`sampling_rate`"):
      meridian.thin_posterior(**kwargs)

  def test_thin_posterior_rejects_invalid_method(self):
    meridian = self._meridian_with_posterior(_simple_posterior())

    with self.assertRaisesRegex(ValueError, "Unsupported"):
      meridian.thin_posterior(n_draws=4, method=mock.MagicMock())

  def test_thin_posterior_rejects_thinning_twice(self):
    meridian = self._meridian_with_posterior(_simple_posterior())

    meridian.thin_posterior(n_draws=4, seed=7)

    with self.assertRaisesRegex(ValueError, "already been thinned"):
      meridian.thin_posterior(n_draws=2)

  def test_thin_posterior_thins_all_matching_groups(self):
    posterior = _simple_posterior(n_chains=3, n_draws=10)
    ...
    meridian = self._meridian_with_posterior(posterior)
    meridian.inference_data.add_groups({"sample_stats": sample_stats})

    thinned = meridian.thin_posterior(n_draws=4, seed=7)

    self.assertEqual(thinned.sizes[constants.DRAW], 4)
    self.assertTrue(thinned.attrs[constants.POSTERIOR_IS_THINNED])

    thinned_sample_stats = meridian.inference_data.sample_stats
    self.assertEqual(thinned_sample_stats.sizes[constants.DRAW], 4)
    self.assertTrue(thinned_sample_stats.attrs[constants.POSTERIOR_IS_THINNED])
    self.assertEqual(
        thinned_sample_stats.attrs[constants.POSTERIOR_THINNING_SEED], 7
    )

    restored_posterior = meridian.restore_full_posterior()
    self.assertEqual(restored_posterior.sizes[constants.DRAW], 10)
    self.assertNotIn(
        constants.POSTERIOR_IS_THINNED, restored_posterior.attrs
    )

    restored_sample_stats = meridian.inference_data.sample_stats
    self.assertEqual(restored_sample_stats.sizes[constants.DRAW], 10)
    self.assertNotIn(
        constants.POSTERIOR_IS_THINNED, restored_sample_stats.attrs
    )
