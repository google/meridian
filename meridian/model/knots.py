...
def get_knot_info(
  ...
  if enable_aks:
    if data is None:
      ...
      aks = AKS()
      knots, _ = aks.automatic_knot_selection(data)
      n_knots = len(knots)
...
class AKS:
  ...
  def calculate_xy_from_input_data(self, data: input_data.InputData):
    ...
    Here, x is a flattened array of indexed time coordinates repeated n_geos
    times (e.g. [0, 1, 2, 3, ... , 0, 1, 2, 3, ...]), and y is the flattened
    array of KPI values that have been population-scaled and mean-centered by
    geo.
