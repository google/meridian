...
class ComputeAdstockHillsTest(
  ...
  def test_adstock_hill_media_invalid_saturation_spec_string(self):
    data = self.input_data_with_media_only
    self.mock_context.input_data = data
    self.mock_context.model_spec = spec.ModelSpec()
    self.mock_context.n_media_times = self._N_MEDIA_TIMES
    ...
    with self.assertRaisesRegex(
      ...
      self.equations.adstock_hill_media(
          ...
          slope=backend.ones(shape=(self._N_MEDIA_CHANNELS,)),
          decay_functions=constants.GEOMETRIC_DECAY,
          saturation_spec="invalid",
      )

  def test_adstock_hill_media_invalid_saturation_spec_sequence(self):
    data = self.input_data_with_media_only
    self.mock_context.input_data = data
    self.mock_context.model_spec = spec.ModelSpec()
    self.mock_context.n_media_times = self._N_MEDIA_TIMES
    ...
    media = backend.to_tensor(data.media, dtype=backend.float_dtype)
    saturation_spec = ["invalid"] + [constants.HILL] * (
  ...
  def test_adstock_hill_media_decay_none_string(self):
    data = self.input_data_with_media_only
    self.mock_context.input_data = data
    self.mock_context.model_spec = spec.ModelSpec(hill_before_adstock=False)
    self.mock_context.n_media_times = self._N_MEDIA_TIMES
    self.mock_context.n_times = self._N_TIMES
    ...
    mock_adstock_output = media * 10.0
    self.enter_context(
        mock.patch.object(
            adstock_hill.AdstockTransformer,
            "forward",
            autospec=True,
            return_value=mock_adstock_output,
        )
    )
    self.enter_context(
        mock.patch.object(
    ...
    result = self.equations.adstock_hill_media(
        ...
        ec=ec,
        slope=slope,
        decay_functions=constants.NONE,
        saturation_spec=constants.HILL,
    )

    test_utils.assert_allclose(result, media)

  def test_adstock_hill_media_saturation_none_string(self):
    data = self.input_data_with_media_only
    self.mock_context.input_data = data
    self.mock_context.model_spec = spec.ModelSpec(hill_before_adstock=False)
    self.mock_context.n_media_times = self._N_MEDIA_TIMES
    ...
    self.enter_context(
        mock.patch.object(
            ...
            "forward",
            autospec=True,
            side_effect=lambda self, x: x * 10.0,
        )
    )
    self.enter_context(
        mock.patch.object(
            ...
            "forward",
            autospec=True,
            side_effect=lambda self, x: x * 5.0,
        )
    )
    ...
    test_utils.assert_allclose(result, mock_adstock_output)
  ...
  def test_adstock_hill_rf_invalid_saturation_spec_string(self):
    data = self.input_data_with_media_and_rf
    self.mock_context.input_data = data
    self.mock_context.model_spec = spec.ModelSpec()
    self.mock_context.n_media_times = self._N_MEDIA_TIMES
  ...
  def test_adstock_hill_rf_invalid_saturation_spec_sequence(self):
    data = self.input_data_with_media_and_rf
    self.mock_context.input_data = data
    self.mock_context.model_spec = spec.ModelSpec()
    self.mock_context.n_media_times = self._N_MEDIA_TIMES
  ...
  def test_adstock_hill_rf_decay_none_string(self):
    data = self.input_data_with_media_and_rf
    self.mock_context.input_data = data
    self.mock_context.model_spec = spec.ModelSpec()
    self.mock_context.n_media_times = self._N_MEDIA_TIMES
    ...
    mock_adstock_output = (reach * frequency) * 10.0
    self.enter_context(
        mock.patch.object(
            adstock_hill.AdstockTransformer,
            "forward",
    ...
    test_utils.assert_allclose(result, reach * frequency)

  def test_adstock_hill_rf_saturation_none_string(self):
    data = self.input_data_with_media_and_rf
    self.mock_context.input_data = data
    self.mock_context.model_spec = spec.ModelSpec()
    self.mock_context.n_media_times = self._N_MEDIA_TIMES
    ...
    expected_output = (reach * frequency) * 10.0
    test_utils.assert_allclose(result, expected_output)
