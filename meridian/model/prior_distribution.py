...
class PriorDistribution:
  ...
  Attributes:
    ...
    total_treatment_allocation_concentration: "Total" concentration parameter
      ...
      This parameter is only used when `use_total_treatment_contribution_prior`
      is `True` and must have a positive prior distribution. Default
      distribution is `Deterministic(100.0)`.
  ...
  def broadcast(
    ...
    sigma_shape = [] if sigma_shape == 1 else sigma_shape
    ...
    if self.total_treatment_allocation_means is not None:
      ...
      if n_paid_channels > 0:
        ...
        paid_mean_total = 1.0 if n_unpaid_channels == 0 else (
            constants.TOTAL_TREATMENT_PAID_MEAN_ALLOCATION
        )
      ...
      if n_unpaid_channels > 0:
        ...
        unpaid_mean_total = 1.0 if n_paid_channels == 0 else (
            1.0 - constants.TOTAL_TREATMENT_PAID_MEAN_ALLOCATION
        )
