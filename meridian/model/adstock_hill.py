...
def transform_non_negative_reals_distribution(distribution):
  """Translates a distribution with support on [0, infinity) to (0, 1].

  Transform a distribution with support on [0, infinity) to (0, 1]. This allows
  for defining a prior on alpha_*, the exponent of the binomial Adstock decay
  function, directly, and then translating it to a distribution defined on the
  unit interval as Meridian expects. This transformation (x -> 1 / (1 + x)) is
  the inverse of the interval mapping the Meridian performs (x -> 1 / x - 1) on
  alpha to define the binomial Adstock decay function's exponent.
  ...
  Args:
    distribution: A Tensorflow Probability distribution with support on
    [0, infinity).
