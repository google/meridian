...
_CHANNEL_FUNCTIONS_TYPE = Mapping[str, str | Sequence[str]] | None


@dataclasses.dataclass(frozen=True)
class AdstockDecayFunctions:
  """Specification for each media channel's adstock decay function.
  ...
  Examples:
    To use binomial decay for all channels:

    ```python
    adstock_decay_functions = AdstockDecayFunctions.from_uniform_type(
        "binomial"
        )
    ```

    To specify binomial decay for "channel_a" and "channel_b" and use the
    default geometric decay for all other channels:

    ```python
      adstock_decay_functions = (
        AdstockDecayFunctions.from_channel_function_mapping({
            "channel_a": "binomial",
            "channel_b": "binomial"
            }))
    ```
  ...
  _channel_functions_map: _CHANNEL_FUNCTIONS_TYPE = None

  @classmethod
  def from_uniform_type(
      cls,
      uniform_decay_function: str = constants.GEOMETRIC_DECAY,
  ) -> 'AdstockDecayFunctions':
    """Create an `AdstockDecayFunctions` with the same value for all channels.

    Arguments:
      uniform_decay_function: A string denoting the adstock decay function
    ...
    adstock_decay_functions = dict.fromkeys(
        constants.ADSTOCK_CHANNELS, uniform_decay_function
        )
    return cls(**adstock_decay_functions)

  @classmethod
  def from_channel_function_mapping(
      cls,
      channel_function_map: Mapping[str, str | Sequence[str]],
      ) -> 'AdstockDecayFunctions':
    """Create an `AdstockDecayFunctions` from a mapping of channels to decay functions.

    Arguments:
      channel_function_map: A mapping of channel names to decay functions.

    """

    return cls(_channel_functions_map=channel_function_map)
  ...
  @property
  def channel_functions_map(self) -> _CHANNEL_FUNCTIONS_TYPE:
    """Mapping from channels to decay functions, used for delayed construction."""
    return self._channel_functions_map
...
def compute_decay_weights(
  ...
  if isinstance(decay_functions, str):
    ...
    return _compute_single_parameterization_decay_weights(
        alpha, l_range, window_size, decay_functions, normalize,
    )

  binomial_weights = _compute_single_parameterization_decay_weights(
      alpha, l_range, window_size, constants.BINOMIAL_DECAY, normalize,
  )
  geometric_weights = _compute_single_parameterization_decay_weights(
  ...
  try:
    ...
    raise ValueError(
        ...
        f' decay_parameterization ({len(decay_functions)})'
    ) from e


def _compute_single_parameterization_decay_weights(
    alpha: backend.Tensor,
    l_range: backend.Tensor,
    window_size: int,
  ...
  Args:
      ...
      window_size: The number of time periods that go into the adstock weighted
        average for each output time period.
      decay_parameterization: String indicating the decay function to use for
        the Adstock calculation. Allowed values are 'geometric' and 'binomial'.
      normalize: A boolean indicating whether to normalize the weights.
...
def _adstock(
  ...
  weights = compute_decay_weights(
      ...
      decay_functions=decay_parameterization,
      normalize=True,
  )
