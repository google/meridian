...
class ModelSpec:
  ...
  Attributes:
    ...
    rf_roi_calibration_period: An optional boolean array of shape
      ...
      the counterfactual where media is set to historical values versus the
      counterfactual where media is set to zero during the calibration period
      and set to historical values for all other time periods. The denominator
      is the channel spend during calibration period (excluding any calibration
      time periods prior to the first KPI time period). Spend data by time
      period is required. If `None`, all times are used. Only used if
