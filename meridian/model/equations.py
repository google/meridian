...
def _process_decay_functions(
  ...
  Returns:
    ...
    decay functions (defaulting to geometric when 'none' is specified, as the
    mask handles bypassing).
  ...
  if isinstance(decay_functions, str):
    if decay_functions not in valid_decay_funcs:
      raise ValueError(
    ...
    decay_mask = backend.to_tensor(
        tuple(d != "none" for d in decay_functions), dtype=backend.bool_
    )
    adstock_decay_functions = [
        constants.GEOMETRIC_DECAY if d == "none" else d for d in decay_functions
    ]
...
class ModelEquations:
  ...
  def adstock_hill_media(
    ...
    if self._context.model_spec.hill_before_adstock:
      ...
      adstock_times = adstock_out.shape[-2]
      ...
      return backend.where(
          decay_mask,
          adstock_out,
          selected_media[..., selected_media.shape[-2] - adstock_times :, :],
      )

    raw_adstock_out = adstock_transformer.forward(media)
    adstock_times = raw_adstock_out.shape[-2]
    # Slicing is used to align the time dimension of raw media with the
    # adstock output window when adstock is bypassed.
    adstock_out = backend.where(
        decay_mask,
        raw_adstock_out,
        media[..., media.shape[-2] - adstock_times :, :],
    )
  ...
  def adstock_hill_rf(
    ...
    decay_mask, adstock_decay_functions = _process_decay_functions(
        decay_functions, frequency.shape[-1]
    )
    ...
    return backend.where(
        decay_mask,
        rf_out,
        rf_media[..., rf_media.shape[-2] - adstock_times :, :],
    )
