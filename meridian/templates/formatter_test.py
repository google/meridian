...
class FormatterTest(parameterized.TestCase):
  ...
  def test_create_card_html_stats_findings(self):
    """Tests that errors, warnings, and infos render inside a stat block."""
    template_env = formatter.create_template_env()
    card_spec = formatter.CardSpec(id='test_id', title='test_title')
    stats_spec = formatter.StatsSpec(
        title='stats_title', stat='test_stat', errors=['Stat Error']
    )
    card_html = ET.fromstring(
        formatter.create_card_html(
            template_env, card_spec, insights=None, stats_specs=[stats_spec]
    ...
    stats_section = card_html.find('stats-section')
    self.assertIsNotNone(stats_section)
    stat_item = stats_section.find('stats')
    self.assertIsNotNone(stat_item)

    error_elem = stat_item.find('errors')
    self.assertIsNotNone(error_elem)
    error_p = error_elem.find('p')
    self.assertIsNotNone(error_p)
    self.assertIn('Stat Error', error_p.text)
