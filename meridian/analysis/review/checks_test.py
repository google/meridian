...
class ConvergenceCheckTest(parameterized.TestCase):
  ...
  @parameterized.named_parameters(
      ...
      dict(
          ...
          rhat_mock_value=9.0,
          expected_case=results.ConvergenceCases.NEEDS_REVIEW,
      ),
      dict(
  ...
  def test_convergence_check(
    ...
    if result.case == results.ConvergenceCases.CONVERGED:
      self.assertIsNone(result.action_items)
      self.assertEqual(
          reporting,
          "The model has likely converged, as all parameters have R-hat values"
          " < 1.2.",
      )
    elif result.case == results.ConvergenceCases.NEEDS_REVIEW:
      self.assertEqual(result.action_items, results.CONVERGENCE_ACTION_ITEMS)
      self.assertEqual(
          reporting,
