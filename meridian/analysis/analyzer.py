...
class Analyzer:
  ...
  def negative_baseline_probability(
    ...
    The baseline is calculated by computign `expected_outcome` with the
    ...
    posterior_baseline_draws = self._calculate_baseline_expected_outcome(
        non_media_baseline_values=non_media_baseline_values,
        use_posterior=use_posterior,
        selected_geos=selected_geos,
        selected_times=selected_times,
    ...
    return np.mean(posterior_baseline_draws < 0)
