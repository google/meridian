...
class AnalyzerTest(tf.test.TestCase, parameterized.TestCase):
  ...
  def test_incremental_outcome_missing_new_param_raises_exception(
    ...
    with self.assertRaisesRegex(
        ...
        " provided in `new_data` with a different number of time periods than"
        " in `InputData`, then all new parameters originally in `InputData`"
  ...
  def test_incremental_outcome_flexible_times_selected_times_wrong_type(self):
    with self.assertRaisesRegex(
        ...
        " `organic_frequency`, `non_media_treatments`, or `revenue_per_kpi` is"
        " provided in `new_data` with a different number of time periods than"
        " in `InputData`, then `selected_times` and `media_selected_times` must"
        " be a list of booleans with length equal to the number of time periods"
        " in the new data.",
    ):
      self.analyzer_media_and_rf.incremental_outcome(
          new_data=analyzer.DataTensors(
  ...
  def test_incremental_outcome_wrong_n_channels_raises_exception(self):
    with self.assertRaisesRegex(
        ValueError,
        "New `reach` is expected to have third dimension of size 2. Actual size"
        " is 1.",
    ):
...
class AnalyzerMediaOnlyTest(tf.test.TestCase, parameterized.TestCase):
  ...
  @parameterized.named_parameters(
      (
          ...
          "media",
          (
              "If `revenue_per_kpi` is provided in `new_data` with a different"
              " number of time periods than in `InputData`, then all new"
              " parameters originally in `InputData` must be provided with the"
      ...
      (
          ...
          (
              ...
              " or `revenue_per_kpi` is provided in `new_data` with a different"
              " number of time periods than in `InputData`, then all new"
              " parameters originally in `InputData` must be provided with the"
              " same number of time periods."
          ),
...
class AnalyzerNonMediaTest(tf.test.TestCase, parameterized.TestCase):
  ...
  def test_incremental_outcome_wrong_non_media_raises_exception(self):
    with self.assertRaisesRegex(
        ValueError,
        "If `non_media_treatments` is provided in `new_data` with a different"
        " number of time periods than in `InputData`, then all new parameters"
        " originally in `InputData` must be provided with the same number of"
        " time periods.",
    ):
...
class AnalyzerOrganicMediaTest(tf.test.TestCase, parameterized.TestCase):
  ...
  @parameterized.named_parameters(
      (
          ...
          "media",
          (
              "New `reach` is expected to have 52 time periods. Found 10 time"
              " periods."
          ),
      ),
      (
